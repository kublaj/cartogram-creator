...
/***/ function(module, exports, __webpack_require__) {
	...
	var _geojson = __webpack_require__(3);

	var _geojson2 = _interopRequireDefault(_geojson);

	var _d3 = __webpack_require__(4);
	...
	var grid = {
	    cells: [],
	    cellSize: 20,
	    isUsed: function isUsed(x, y) {
	        return this.cells.some(function (cell) {
	            return cell.x === x && cell.y === y;
	        });
	    },
	    use: function use(x, y) {
	        this.cells.push({ x: x, y: y });
	    },
	    getInterestingCells: function getInterestingCells() {
	        var _this = this;

	        var iCells = this.cells.map(function (cell) {
	            return makeCell(cell.x, cell.y, _this.cellSize, true);
	        });
	        this.cells.forEach(function (cell) {
	            for (var x = cell.x - 1; x <= cell.x + 1; x++) {
	                for (var y = cell.y - 1; y <= cell.y + 1; y++) {
	                    if (!iCells.some(function (iCell) {
	                        return iCell.x === x && iCell.y === y;
	                    })) {
	                        iCells.push(makeCell(x, y, _this.cellSize, false));
	                    }
	                }
	            }
	        });
	...
	grid.use(0, 0);
	console.log(grid.getInterestingCells());
	...
	var Node = (function () {
	    ...
	    _createClass(Node, [{
	        key: 'assignCell',
	        value: function assignCell(cell) {
	            this.cell = cell;
	        }
	    }, {
	        key: 'isAssigned',
	...
	var projection = _d32['default'].geo.mercator();

	//Define path generator
	var path = _d32['default'].geo.path().projection(projection);

	var mapCenter = path.centroid(_geojson2['default']);

	var nodes = _geojson2['default'].features.map(function (el) {
	    var coords = path.centroid(el);
	    var scale = 1000;
	    return new Node((coords[0] - mapCenter[0]) * scale, (coords[1] - mapCenter[1]) * scale);
	});

	var mapBounds = nodes.reduce(function (prev, node) {
	    return {
	...
	var centralNode = nodes.reduce(function (bestCandidate, node) {
	...
	var viewBox = mapBounds.x1 + ' ' + mapBounds.y1 + ' ' + (mapBounds.x2 - mapBounds.x1) + ' ' + (mapBounds.y2 - mapBounds.y1);
	svg.attr({
	    viewBox: viewBox
	});

	centralNode.matrixX = 0;
	centralNode.matrixY = 0;
	centralNode.allocated = true;
	grid.use(0, 0);

	/*


	 - get all allocated and neighbouring matrix cells

	 */

	function getCheckCells() {
	    var cells = [];
	    for (var i = 0, l = nodes.length; i < l; i++) {
	        ...
	        if (node.allocated) {
	            ...
	            var my = node.matrixY;
	            console.log(mx, my);
	            for (var xx = mx - 1; xx <= mx + 1; xx++) {
	                for (var yy = my - 1; yy <= my + 1; yy++) {
	                    if (cells.indexOf(xx + ',' + yy) === -1) {
	                        console.log(xx);
	                        cells.push(xx + ',' + yy);
	                    }
	                }
	    ...
	    return cells.map(function (cell) {
	        ...
	        return {
	            ...
	            y: +coords[1],
	            //   used: coords[2] === 'true',
	            x1: (coords[0] - 0.5) * cellSize,
	            x2: (coords[0] + 0.5) * cellSize,
	            y1: (coords[1] - 0.5) * cellSize,
	            y2: (coords[1] + 0.5) * cellSize
	        };
	    });
	}
	...
	function getNodesInCells(cells) {
	    ...
	    nodes.forEach(function (node) {
	        ...
	        for (var i = 0; i < cells.length; i++) {
	            var cell = cells[i];
	            console.log(node.x, node.y, cell.x1, cell.x2, cell.y1, cell.y2);
	...
	function getNextNode() {
	    ...
	    do {
	        nodesInCells = getNodesInCells(getCheckCells());
	        if (nodesInCells.length < 1) {
	            directionChange = direction !== (direction = 1);
	            step = directionChange ? step / 2 : step;
	            cellSize += step;
	        } else if (nodesInCells.length > 1 || nodesInCells[0].used) {
	            directionChange = direction !== (direction = -1);
	            step = directionChange ? step / 2 : step;
	            cellSize -= step;
	        }
	    } while (nodesInCells.length !== 1 /* || nodesInCells[0].used*/);
	    return nodesInCells[0].node;
	}

	function allocateAllNodes() {
	    while (nodes.some(function (node) {
	...
	function useCell(node, cellX, cellY) {
	    node.allocated = true;
	    node.matrixX = cellX;
	    node.matrixY = cellY;
	}
	...
	function drawAll() {
	    ...
	    svg.selectAll('path').data(nodes).enter().append('circle').style('fill', function (d) {
	    ...
	    svg.selectAll('path').data(nodes).enter().append('rect').filter(function (d) {
	        return d.allocated;
	    }).style('fill', function (d) {
	        return 'rgba(0,0,255,.5)';
	    }).attr({
	        x: function x(d) {
	            return d.matrixX * cellSize;
	        },
	        y: function y(d) {
	            return d.matrixY * cellSize;
	        },
	        width: cellSize + 'px',
	        height: cellSize + 'px'
	    });
